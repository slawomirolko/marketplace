...
file under the Android project directory changed, step 5 (Run tests) is NOT complete until the
FULL instrumentation suite ran on the Docker emulator(`:app:connectedDebugAndroidTest`, NO class filters — whole suite, every
...
Compose-managed emulator service (started by the repository's emulator lifecycle
script) is the required emulator; a missing
