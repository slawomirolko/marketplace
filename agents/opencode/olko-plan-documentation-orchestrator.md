...
2. Collect all worker results. Load `olko-plan-editor` through the `skill` tool and create or update the technical document (`implementation.md`) from verified worker evidence. Preserve the business writer's document unless a verified fact requires a correction. Add an `## Implementation readiness` section containing the readiness worker's evidence and a final `Ready for implementation: yes/no` decision. A `yes` requires every architecture, dependency, instrumentation-design, runtime-verification-contract, and error-handling item to be `ready`, or an explicit user-approved exception. A `yes` ALSO requires ZERO open items: no unanswered user decision, no unresolved blocker, no unresolved assumption surfaced by the grill or blocker-grill, and no open question that could change scope, mechanism, contract, planned files, or acceptance criteria. The technical document MUST carry an `## Open items` section listing every open item classified as BLOCKING, VERIFY-DURING-WORK, or ACCEPTED-GAP, and a `yes` verdict is FORBIDDEN while any item is BLOCKING or while any ACCEPTED-GAP item lacks an explicit user acceptance recorded in `tracker.md`. VERIFY-DURING-WORK items are environmental checks resolved by performing the work itself and may remain under a `yes` only when they cannot change what is built. Never report a `no` with remaining blockers as completion. The saved pair + `tracker.md`, not chat prose, is the output of this stage.
